...
if sym == 4:
    resultSurfaceEvolver = scipy.io.loadmat('ResultsCombined_L4_amp2_arcAngle30_ccDist301-350um-count50_rotAngle91_bathRad500.mat')
...
energySE = resultSurfaceEvolver['netEnergy_2Rafts_reZero'].transpose() # unit: fJ, [301-350um, 0-90deg], rezeroed by energy at 45 deg & 350 um. 
...
angleAveragedForceNormalizedSE = resultSurfaceEvolver['angleAveragedForceNormalized'].transpose() # normalized by 74 mN/m * 2 um = 148 nN
angleAveragedNetEnergySE = resultSurfaceEvolver['angleAveragedNetEnergy'].transpose() # unit: fJ
angleAveragedNetEnergyNormalizedSE =  resultSurfaceEvolver['angleAveragedNetEnergyNormalized'].transpose() # unit: normalized by 74 mN/m * 2 um * 2 um = 296 fJ
...
rotationAngleTheory = np.arange(resultTheory['thetaStart'][0][0], resultTheory['thetaEnd'][0][0] + 1, resultTheory['thetaStepSize'][0][0])
energyTheory = resultTheory['energyScaledToRealSurfaceTensionRezeroed'] # unit: fJ, [41-1000um, 0-90deg], rezeroed by energy at 45 deg & 350 um. 
...
angleAveragedForceNormalizedTheory = resultTheory['angleAveragedForceNormalized'] # normalized by 74 mN/m * 2 um = 148 nN
angleAveragedNetEnergyTheory = resultTheory['angleAveragedNetEnergy'] # unit: fJ
angleAveragedNetEnergyNormalizedTheory =  resultTheory['angleAveragedNetEnergyNormalized'] # unit: normalized by 74 mN/m * 2 um * 2 um = 296 fJ
...
if sym == 4:
    ...
    anglesForMaxForce = np.argmax(forceCombinedDistancesAsRows[:,:45], axis = 1) # max force is positive repulstion, unit: degree
    anglesForMinForce = np.argmin(forceCombinedDistancesAsRows[:,:50], axis = 1) # min force is negative, attraction unit: degree
...
#extends to all 360 deg, the torque at the begining and the end needs to readjusted to 0 because those values are due to one-sided gradient.  
if sym == 4:
    forceCombinedDistancesAsRowsAll360 = np.hstack((forceCombinedDistancesAsRows[:,:90], forceCombinedDistancesAsRows[:,:90], forceCombinedDistancesAsRows[:,:90], forceCombinedDistancesAsRows[:,:90])) 
    torqueCombinedDistancesAsRows[:,0] = 0
    torqueCombinedDistancesAsRows[:,-1] = 0
    torqueCombinedDistancesAsRowsAll360 = np.hstack((torqueCombinedDistancesAsRows[:,:90], torqueCombinedDistancesAsRows[:,:90], torqueCombinedDistancesAsRows[:,:90], torqueCombinedDistancesAsRows[:,:90]))
elif sym == 6:
    forceCombinedDistancesAsRowsAll360 = np.hstack((forceCombinedDistancesAsRows[:,:60], forceCombinedDistancesAsRows[:,:60], forceCombinedDistancesAsRows[:,:60], forceCombinedDistancesAsRows[:,:60], forceCombinedDistancesAsRows[:,:60], forceCombinedDistancesAsRows[:,:60]))
    torqueCombinedDistancesAsRows[:,0] = 0
    torqueCombinedDistancesAsRows[:,-1] = 0
    torqueCombinedDistancesAsRowsAll360 = np.hstack((torqueCombinedDistancesAsRows[:,:60], torqueCombinedDistancesAsRows[:,:60], torqueCombinedDistancesAsRows[:,:60], torqueCombinedDistancesAsRows[:,:60], torqueCombinedDistancesAsRows[:,:60], torqueCombinedDistancesAsRows[:,:60]))

outputDataFileName = 'capillaryForceAndTorque_sym' + str(sym) 
listOfVariablesToSave = ['eeDistanceCombined', 'forceCombinedDistancesAsRowsAll360', 'torqueCombinedDistancesAsRowsAll360']
